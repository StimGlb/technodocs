...
N'utilise jamais git commit directement. Utilise exclusivement le script de commit du projet via npm run commit (ou npm run commit:win sur Windows). Pour la branche security-dev, si une divergence est détectée, demande confirmation avant toute opération de force push.
...
- **Branch Context**: Always verify you are on `security-dev` before modifying `dist/js/`.
