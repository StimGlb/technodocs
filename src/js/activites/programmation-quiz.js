/**
 * Quiz Programmation 4e/3e — TechnoDocs
 * Collection Firestore : quiz_submissions
 * wizard_id : programmation-quiz-4e-3e
 *
...
const ATTEMPT_KEY = 'quiz_programmation-4e3e_attempts';
...
function validateQuestion(questionId) {
  ...
  // Verrouiller les options et colorer
  optionsEl.querySelectorAll('.wizard__option').forEach((btn) => {
    btn.disabled = true;
  ...
  // Afficher le feedback (la classe CSS gère quel paragraphe est visible)
  if (feedbackEl) {
    feedbackEl.classList.add('wizard__feedback--visible');
    feedbackEl.classList.add(
      isCorrect ? 'wizard__feedback--correct' : 'wizard__feedback--incorrect',
    );
    // Maintenir la position : scroll vers le feedback sans remonter en haut
...
function showBilan() {
  ...
  const attemptNumber = getAttempts() + 1; // +1 car pas encore incrémenté
  ...
  let message;
  if (state.score === 8) {
    message = '🏆 Excellent ! Tu maîtrises les bases de la programmation.';
  ...
  // Afficher l'avertissement de pénalité si 2e tentative
  if (penaltyEl) {
    if (isSecondAttempt) {
...
async function saveToFirestore(savedScore, attemptNumber) {
  ...
  try {
    ...
    await setDoc(docRef, {
      ...
      score_brut: state.score,          // score réel obtenu
      score: savedScore,                // score retenu (× 0,5 si 2e tentative)
    ...
    console.error('❌ Erreur Firestore (quiz):', err);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Vérification quota dès le chargement — désactive le bouton si épuisé
  ...
  // Initialiser chaque question
  Object.keys(CORRECT_ANSWERS).forEach((qId) => initQuestion(qId));

  // Boutons "Question suivante" / "Voir mon bilan"
